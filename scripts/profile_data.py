...
def profile(path):
    ...
    for c in cols:
        ...
        if numeric:
            ...
            if ints and 1500 <= min(ns) <= 2100 and 1500 <= max(ns) <= 2100 and distinct > 3: info["role"] = "year"
            elif distinct <= 12 and ints: info["role"] = "ordinal/category"
            else: info["role"] = "measure"
        else:
            info["type"] = "text"
            if all(ISO3.match(str(v)) for v in nonblank[:200]) and distinct > 20: info["role"] = "iso3"
            elif re.match(r"^\d{4}-\d{2}(-\d{2})?", str(nonblank[0])): info["role"] = "date"
            elif distinct == len(nonblank): info["role"] = "id (unique)"
    ...
    entities = [c["name"] for c in out["columns"] if c.get("role") in ("entity (many)", "category", "id (unique)")]
    iso = [c["name"] for c in out["columns"] if c.get("role") == "iso3"]
    shape = "long (entity × year)" if years and entities else "wide (one row per period)" if years else "cross-section (one row per entity)" if entities else "unknown"
    out["shape"] = shape
    sug = []
