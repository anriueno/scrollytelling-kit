...
export function createLine(container, spec, datasets) {
  ...
  return {
    render(state, immediate) {
      ...
      const top = 78, gapX = 44, gapY = 56, padR = facets.length > 1 ? 8 : 130;
      ...
      pm.each(function (p) {
        ...
        dl.enter().append("text").attr("class", "label strong").attr("opacity", 0).merge(dl).text((d) => `${labelFor(spec, d.key)} ${f(d.v)}`).transition().duration(t).attr("x", (d) => d.lx).attr("y", (d) => d.ly).attr("opacity", 1); dl.exit().remove();
      ...
      P.exit().remove();
      ch.legend(keys.map((k) => [labelFor(spec, k), colorFor(spec, k, keyIdx(k))])); ch.legendLeft(20);
    }, resize() {}, progress() {},
  };
}
