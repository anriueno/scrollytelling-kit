...
export function createMap(container, spec, datasets, { geo }) {
  ...
  pat.append("rect").attr("width", 6).attr("height", 6).attr("fill", "#1c2029"); pat.append("line").attr("x1", 0).attr("y1", 0).attr("x2", 0).attr("y2", 6).attr("stroke", "#2f3540").attr("stroke-width", 2);
  ...
  const scale = d3.scaleSequential(d3.interpolateLab(...(spec.colors || ["#4a3f2c", "#ffb03b"]))).domain(spec.domain || d3.extent(rows, (r) => num(r[spec.value]))).clamp(true);
